...
st.caption(
    "Versión base: índices, parámetros y variables indexadas."
)
...
def is_valid_symbol(name: str) -> bool:
    """
    Valida nombres tipo símbolo algebraico:
    - no vacío
...
def build_index_elements(size: int, label_prefix: str) -> List[str]:
    """
    Genera etiquetas para un índice.
    Ejemplo: i con tamaño 3 -> ['i1','i2','i3']
    """
...
def cartesian_labels(index_names: List[str], index_specs: Dict[str, Dict[str, Any]]) -> List[Tuple[str, ...]]:
    """
    Devuelve el producto cartesiano de etiquetas de los índices.
    Ejemplo: ['i','j'] -> [('i1','j1'), ('i1','j2'), ...]
    """
    if len(index_names) == 0:
        return [tuple()]
...
def dims_shape(index_names: List[str], index_specs: Dict[str, Dict[str, Any]]) -> List[int]:
    return [int(index_specs[idx]["size"]) for idx in index_names]


def dims_labels(index_names: List[str], index_specs: Dict[str, Dict[str, Any]]) -> List[List[str]]:
    return [index_specs[idx]["elements"] for idx in index_names]
...
def reset_parameters_if_invalid(index_specs: Dict[str, Dict[str, Any]]) -> None:
    """
    Si se elimina un índice o cambia su nombre, depura referencias inválidas.
    """
...
st.write(
    "Define los índices del modelo. Ejemplos: `i`, `j`, `k`."
)
...
if valid_indices and len(index_specs) > 0:
    st.subheader("Vista previa de índices")
    preview_idx = []
    ...
    st.dataframe(pd.DataFrame(preview_idx), use_container_width=True)
...
if not valid_indices or len(index_specs) == 0:
    ...
    st.write(
        "Define parámetros escalares, vectores, matrices o tensores pequeños. "
    ...
    if len(new_params_spec) > 0:
        ...
        with st.expander("Ver valores de parámetros"):
            for pname, pspec in new_params_spec.items():
                st.markdown(f"**{pretty_param_signature(pname, pspec['indices'])}**")
                df_preview = parameter_preview_dataframe(pname, pspec, index_specs)
...
if not valid_indices or len(index_specs) == 0:
    ...
    for v in range(int(num_vars)):
        ...
        total_size = total_elements_for_dims(v_indices, index_specs)
        st.write(
            f"**Firma:** `{pretty_var_signature(vname, v_indices)}`  \n"
            f"**Dominio:** `{infer_domain_text(v_domain)}`  \n"
            f"**Número total de componentes:** `{total_size}`"
        )
...
# ============================================================
# SECCIÓN 4: VISTA PREVIA DE LA ESTRUCTURA DEL MODELO
# ============================================================

st.markdown("---")
st.header("4. Vista previa de la estructura")
...
# ============================================================
# SECCIÓN 5: EXPORTACIONES ÚTILES
# ============================================================

st.markdown("---")
st.header("5. Exportaciones rápidas")
...
# ============================================================
# NOTAS FINALES EN LA INTERFAZ
# ============================================================

st.markdown("---")
st.info(
    "Esta versión ya construye la base del modelo: índices, parámetros y variables. "
    "El siguiente paso correcto es añadir el constructor interactivo de la función objetivo "
    "y después el constructor de familias de restricciones."
)
